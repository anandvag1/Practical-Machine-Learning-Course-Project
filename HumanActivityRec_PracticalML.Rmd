...
To check the model fit and out of sample error we will do 10 fold cross validation which can also be input to train method.
...
We will use random forest model with 15 trees to predict on testing set.

```{r predict, echo=TRUE}
predict(modellist[[15]], pml_testing)
```
